...
import { CLANGD_PATH, SERVER_HOST } from './config';

/**
 * start the language server inside the current process
 */
export const launchLanguageServer = (serverName: string, socket: IWebSocket) => {
  // start the language server as an external process

  const clangdPath = CLANGD_PATH;
...
export const upgradeWsServer = (config: {
  ...
  config.server.on('upgrade', (request: IncomingMessage, socket: Socket, head: Buffer) => {
    const baseURL = `http://${SERVER_HOST || request.headers.host}/`;
